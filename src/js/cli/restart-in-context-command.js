...
export default (cli) => {
  cli.command(
    ...
    (
      <Paragraph margin='none'>
        Here is an example: <code>restart --buildNumber=10</code>
      </Paragraph>
    )
  )
  .option(
  ...
  .action((args, context) => {
    const repoName = context;
    let repoMatch;
    store.getState().user.repos.some((repo) => {
      if (repo.name === repoName || repo.full_name === repoName) {
        repoMatch = repo;
      }
      return repoMatch;
    });
    ...
    if (repoMatch) {
      if (repoMatch.status === 'pending' || repoMatch.status === 'running') {
        return Promise.reject(
          `You need to wait, ${repoName} is already ${repoMatch.status}.`
        );
      }
      return getBuilds(repoMatch.full_name).then(
        (builds) => {
          if (!builds || builds.length === 0) {
            return Promise.reject(
              `Bad news, ${repoName} does not have any builds to restart.`
            );
          }
          let buildMatch;
    ...
    return Promise.reject(`Well, I could not find ${repoName} project.`);
  });
};
